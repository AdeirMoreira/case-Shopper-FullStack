...
export class StockBusiness {
	...
	public update = async (productsPurchasedDto: ProductsPurchasedDto) => {
		try {
			...
			await this.CheckProductsPurchasedConformity(productsPurchasedDto.productsPurchased as any);

			const productsPurchasedDB = await this.checkProductsInDataBase(
				productsPurchasedDto.productsPurchased as any
			);

			const productsDto: ProductsPurchased = productsPurchasedDto.productsPurchased as any;
			const productsUpdatedArray = [];
			for (let index in productsPurchasedDB) {
				const newProductQty = productsPurchasedDB[index].qty_stock - productsDto[index].qty_purchased;
				if (newProductQty < 0) {
	...
	private async CheckProductsPurchasedConformity(productsPurchased: ProductsPurchased[]) {
	...
	private async checkProductsInDataBase(productsPurchased: ProductsPurchased[]) {
